  ...
  async execute(client) {
    ...
    (async () => {
      try {
        ...
        await rest.put(Routes.applicationGuildCommands(clientId, paradoxia), {
          body: commands,
        });
        ...
        axo.startupMsg(`╠═════════════════════════════════════════════════╦`);
        axo.startupMsg(`║ Johnbot OS by SimpNotFound#3742                 ║`);
        axo.startupMsg(
          `║ My current ping to Discord is ${client.ws.ping.toString()} ms            ║`
        );
        axo.startupMsg(`╚═════════════════════════════════════════════════╝`);
      } catch (error) {
        axo.err(error);
      ...
      //sets mongo url
      cs.setMongoURL(
        "mongodb+srv://cheese:3Pphi5G8EtzNxhzv@sussycoins.qyewh.mongodb.net/myFirstDatabase?retryWrites=true&w=majority"
      );
